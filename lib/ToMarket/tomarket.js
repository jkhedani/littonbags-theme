   ...
   *				  setting other than color can differentiate the same item.
	...
	var refresh_handbasket = function( type ) {
		...
		if ( simpleStorage.index().length < 1 ) {
			...
			// Generate totals & tax
			var tax_rate_dollars = Math.round( to_market_scripts.tax_rate * handbasket_subtotal );
			var grand_total = Math.round( tax_rate_dollars + handbasket_subtotal );
	...
	$(document).on('click', '[data-toggle="hand-basket"]', function() {
		...
		$('.back-to-top').click(); // Scroll back to top
	});

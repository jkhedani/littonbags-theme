jQuery(document).ready(function($){
	 ...
	 $(document).on('click', 'a.checkout', function() {
			...
			$.post(shopping_cart_scripts.ajaxurl, {
				...
				if (response.success===true) {
					...
					$('#checkoutModal').find('.modal-body .checkoutPay input[name="description"]').attr('value',response.desc);
					return false;
				} else {
	...
	$('#submitBasicInfo').on( 'click', function() {
		...
		if ( ! $('#shippingIsDifferent').is(':checked') ) {
			$('input[data-stripe="address-line1"]').val( $('input[data-easypost="shipping-address-line1"]') );
			$('input[data-stripe="address-line2"]').val( $('input[data-easypost="shipping-address-line2"]') );
			$('input[data-stripe="address-city"]').val( $('input[data-easypost="shipping-address-city"]') );
			$('input[data-stripe="address-zip"]').val( $('input[data-easypost="shipping-address-zip"]') );
			$('input[data-stripe="address-state"]').val( $('input[data-easypost="shipping-address-state"]') );
			$('input[data-stripe="address-country"]').val( $('input[data-easypost="shipping-address-country"]') );
		}
	...
	/**
	 *	C. Show Purchase Success Screen (should this be more generice)
	 */
	...
	if($_GET["payment"] == 'paid') {
		...
		$('#checkoutModal .checkoutThanks').show('fast');
	}

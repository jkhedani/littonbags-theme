jQuery(document).ready(function($){
				...
				if ( $.jStorage.index().length ) {
					...
					// Create a shopping object for each product in the popover shopping cart
						...
						$('.shoppingcart a.shoppingcartshow').popover({
							'html': true,
							'placement': 'bottom',
							'content': "<h1>Shopping Cart</h1><div class='shopping-cart-preview-legend'><span class='product-title'>Product Name</span><span class='product-price'>Unit Price</span><span class='product-qty'>Qty</span><span class='product-subtotal'>Subtotal</span><span class='product-remove'>Remove</span></div>"+response.html+"<hr /><a class='checkout'>Checkout</a>",
	...
	$('#submitBasicInfo').on( 'click', function() {
		...
		if ( stripePreparationHandler('basic') ) {
			...
			// Global constants
			var ajaxSpinner = $('body').find('.modal-footer .checkoutBasic .processing-spinner');
